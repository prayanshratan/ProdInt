...
export async function createTemplate(templateData: Omit<PRDTemplate, 'id' | 'createdAt'>): Promise<PRDTemplate> {
  ...
  // If this is set as default, unset other defaults for this user of the same type
  if (templateData.isDefault) {
    for (const template of templates) {
      if (template.userId === templateData.userId &&
        template.isDefault &&
        (template.type === templateData.type || (!template.type && templateData.type === 'prd'))) { // Handle legacy templates as PRD
  ...
  const newTemplate: PRDTemplate = {
    ...templateData,
    type: templateData.type || 'prd', // Default to PRD for safety
    id: Date.now().toString(),
    createdAt: new Date().toISOString(),
  }
...
export async function updateTemplate(id: string, updates: Partial<PRDTemplate>): Promise<PRDTemplate> {
  ...
  // If setting as default, unset other defaults for this user of the same type
  if (updates.isDefault) {
    const userId = templates[index].userId
    const type = updates.type || templates[index].type || 'prd'

    for (const template of templates) {
      const tType = template.type || 'prd'
